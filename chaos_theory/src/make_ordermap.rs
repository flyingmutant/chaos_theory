    ...
    fn ordermap_smoke() {
        ...
        check(|src| {
            prop_smoke(
                src,
                "OrderSet",
                make::ordermap::order_set_n::<_, RandomState>(make::arbitrary::<i32>(), ..MAX_SIZE),
            );
            prop_smoke(
                src,
                "OrderMap",
                make::ordermap::order_map_n::<_, _, RandomState>(
                    make::arbitrary::<i32>(),
                    make::arbitrary::<i32>(),
                    ..MAX_SIZE,
                ),
