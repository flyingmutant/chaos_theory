...
// Can't match anything because `_` follows `$` (EOL assert)
// minipre-0.2.0: "$_"
...
consistent!(
    ...
    r"((?:(?:0|1[\d]{0,2}|2(?:[0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?)\.){3}(?:0|1[\d]{0,2}|2(?:[0-4]\d?|5[0-5]?|[6-9])?|[3-9]\d?))"
);
...
consistent!(
    tin_drummer_0,
    r".*?\.(a|la|lo|o|ll|keter|bc|dyn_o|d|rlib|crate|min\.js|hi|dyn_hi|S|jsexe|webapp|js\.externs|ibc|toc|aux|fdb_latexmk|fls|egg-info|whl|js_a|js_hi|jld|ji|js_o|so.*|dump-.*|vmb|crx|orig|elmo|elmi|pyc|mod|p_hi|p_o|prof|tix)$"
);
...
consistent!(
    tin_drummer_2,
    r".*?\.(exe|a|la|o|ll|keter|bc|dyn_o|d|rlib|crate|min\.js|hi|dyn_hi|jsexe|webapp|js\.externs|ibc|toc|aux|fdb_latexmk|fls|egg-info|whl|js_a|js_hi|jld|ji|js_o|so.*|dump-.*|vmb|crx|orig|elmo|elmi|pyc|mod|p_hi|p_o|prof|tix)$"
);
...
consistent!(
    os_type_0,
    r"^Microsoft Windows \[Version\s(\d+\.\d+\.\d+)\]$"
);
...
consistent!(
    upm_lib_0,
    r"^(\d+)\.(\d+)\.(\d+)(?:-([\dA-Za-z-]+(?:\.[\dA-Za-z-]+)*))?(?:\+([\dA-Za-z-]+(?:\.[\dA-Za-z-]+)*))?$"
);
...
consistent!(
    woothee_17,
    r"(?:SoftBank|Vodafone|J-PHONE)/[.0-9]+/([^ /;()]+)"
);
...
consistent!(
    woothee_41,
    r"(?:PEAR HTTP_Request|HTTP_Request)(?: class|2)"
);
...
consistent!(
    conserve_0,
    r"^b0000 {21} complete   20[-0-9T:+]+\s +\d+s\n$"
);
...
consistent!(
    rust_enum_derive_2,
    r"^[:space:]*([[:alnum:]_]+)([:space:]*=[:space:]*([:graph:]+))?[:space:]*,"
);
...
consistent!(
    claude_0,
    r"^(?:\b|(-)?)(\p{Currency_Symbol})?((?:(?:\d{1,3}[\.,])+\d{3})|\d+)(?:[\.,](\d{2}))?\b$"
);
...
consistent!(
    raven_0,
    r"^(?P<protocol>.*?)://(?P<public_key>.*?):(?P<secret_key>.*?)@(?P<host>.*?)/(?P<path>.*/)?(?P<project_id>.*)$"
);
...
consistent!(
    parser_haskell_2,
    r"(?m)^#(if|ifn?def|endif|else|include|elif).*"
);
...
consistent!(
    bobbin_cli_8,
    r"(?m)Teensy Loader, Command Line, Version (.*)\n"
);
...
consistent!(
    file_sniffer_2,
    r".*?\.(exe|a|la|o|ll|keter|bc|dyn_o|out|d|rlib|crate|min\.js|hi|dyn_hi|jsexe|webapp|js\.externs|ibc|toc|aux|fdb_latexmk|fls|egg-info|whl|js_a|js_hi|jld|ji|js_o|so.*|dump-.*|vmb|crx|orig|elmo|elmi|pyc|mod|p_hi|p_o|prof|tix)$"
);
...
consistent!(
    slippy_map_tiles_1,
    r"^(?P<minlon>-?[0-9]{1,3}(\.[0-9]{1,10})?) (?P<minlat>-?[0-9]{1,3}(\.[0-9]{1,10})?) (?P<maxlon>-?[0-9]{1,3}(\.[0-9]{1,10})?) (?P<maxlat>-?[0-9]{1,3}(\.[0-9]{1,10})?)$"
);
...
consistent!(
    mozversion_0,
    r"^(?P<major>\d+)\.(?P<minor>\d+)(?:\.(?P<patch>\d+))?(?:(?P<pre0>[a-z]+)(?P<pre1>\d*))?$"
);
...
consistent!(
    skim_12,
    r"\x1B\[(?:([0-9]+;[0-9]+[Hf])|([0-9]+[ABCD])|(s|u|2J|K)|([0-9;]*m)|(=[0-9]+[hI]))"
);
