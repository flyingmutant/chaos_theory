...
enum Commands {
    ...
    /// Generate time zone name tables
    GenTzdb,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::GenTzdb => run_gen_tzdb(&sh),
...
fn run_gen_tzdb(sh: &Shell) -> Result<()> {
    let version = jiff_tzdb::VERSION.context("bundled tzdb has no version")?;
    let mut names: Vec<_> = jiff_tzdb::available().collect();
    names.sort_unstable();
    names.dedup();
    ...
    writeln!(out, "//")?;
    writeln!(out, "//   cargo x gen-tzdb")?;
    writeln!(out, "//")?;
    writeln!(out, "// IANA Time Zone Database version: {version}.")?;
    writeln!(out)?;
    writeln!(
        out,
        "pub(crate) const TIME_ZONE_NAMES_WITH_UNKNOWN: &[&str] = &["
    )?;
    writeln!(
        out,
        "    \"Etc/Unknown\", // CLDR unknown time zone, not an IANA identifier."
    )?;
    for name in names {
        writeln!(out, "    {name:?},")?;
    }
    ...
    writeln!(
        out,
        "#[expect(dead_code, reason = \"reserved for future time library adapters\")]"
    )?;
    writeln!(out, "pub(crate) const TIME_ZONE_NAMES: &[&str] =")?;
    writeln!(out, "    TIME_ZONE_NAMES_WITH_UNKNOWN.split_at(1).1;")?;
    sh.write_file("chaos_theory/gen/time_zone_names.rs", out)?;
