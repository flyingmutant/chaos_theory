...
impl Env {
    ...
    pub(crate) fn choose_size(
        ...
        let n = if self.budget_remaining > 0 {
            (r.max - r.min).min(max_extra).saturating_add(1)
        } else {
            1
        };
    ...
    /// Whether the current region is generated fresh, including regions regenerated by mutation.
    pub(crate) fn is_fresh(&self) -> bool {
        self.tape_replay.is_empty() || self.tape_replay.is_void_reuse()
    }
